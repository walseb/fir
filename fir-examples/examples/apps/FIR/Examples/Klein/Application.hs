...
module FIR.Examples.Klein.Application ( klein ) where
...
import FIR.Examples.Klein.Shaders
...
appName = "fir-examples - Kleinian limit set"
shortName :: String
shortName = "klein" -- name for screenshots
...
klein :: IO ()
klein = runVulkan initialState do
  ...
  -- Create framebuffer attachments.
    ...
    mainLoop do
      ...
      when (locate action) do
        let myfun (V2 px py) = V2 ( (px - 960) / 480 ) ( (1080 - py) / 270 )
        liftIO ( putStrLn ( "pos: " ++ show ( myfun pos ) ) )
