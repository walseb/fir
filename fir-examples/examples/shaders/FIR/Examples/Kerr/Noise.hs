...
hash2 p = purely do
  q <- let' $ Vec2 ( p ^.^ Vec2 127.1 311.7 ) ( p ^.^ Vec2 269.5 183.3 )
  let' $ ( ( \ x -> -1 + 2 * ( sin x * 43758.5453123 `mod` 1 ) ) :: Code Float -> Code Float ) <$$> q

noise :: Code (V 2 Float) -> Program i i (Code Float)
noise v@(Vec2 x y) = purely do
  i@(~(Vec2 ix iy))
     <- let' $ ( floor :: Code Float -> Code Float ) <$$> ( v ^+^ pureAST ( ( x + y ) * k1 ) )
  a@(~(Vec2 ax ay)) <- let' $ v ^-^ i ^+^ pureAST ( (ix + iy) * k2 )
  s <- let' $ signum ( ax - ay )
  o@(~(Vec2 ox oy)) <- let' $ Vec2 ( 0.5 + 0.5 * s ) ( 0.5 - 0.5 * s )
