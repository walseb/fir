...
eventColour kerrInfo constants x₀ evtData
  = purely do
      flag <- let' $ view @(Field "flag") evtData
      if flag < 1 -- black
      then pure (Vec4 0 0 0 (Lit (1/0)))
      else do
        value     <- let' $ view @(Field "value"    ) evtData
        intensity <- let' $ view @(Field "intensity") evtData
  ...
  RK.dormandPrince
    ...
    where
      ...
      -- For volumetric rendering we would want to keep track of the past.
        = purely do
            dλ <- let' $ fst ( RK.stepSize stepData )
...
crossedEventHorizon
  ...
  = purely do
      ...
      then do
        ...
        r_h <- let' $ view @(Field "r_h") kerrInfo
        if r + dλ * r' < r_h + 1e-5
        then do
...
crossedAccretionDisk
  ...
  = purely do
      ...
      r_inner <- let' $ view @(Field "r_inner"   ) diskInfo
      r_outer <- let' $ view @(Field "r_outer"   ) diskInfo
      θ_disk  <- let' $ view @(Field "thickness" ) diskInfo
      ω_prec  <- let' $ view @(Field "precession") diskInfo

      -- Linear approximation of disk tilt around r.
      σ    <- let' $ acos cosθ - pi / 2
      sinθ <- let' $ sqrt ( 1 - cosθ * cosθ )
      cosψ <- let' $ cos ( φ + ( clock - λ ) * ω_prec )

      r_min <- let' $ max r_inner ( min r (r + 1.1 * dλ * r') )
      r_max <- let' $ min r_outer ( max r (r + 1.1 * dλ * r') )

      τ_min <- let' =<< tilt diskInfo r_min
      τ_max <- let' =<< tilt diskInfo r_max

      -- AABB intersection test
      invDir <- let'$ recip @(Code Float) <$$> Vec2 r' ( - cosθ' / sinθ )
      t1 <- let'
      ...
      t2 <- let'
        $ ( \ s p i -> ( s - p ) * i :: Code Float )
            ...
            <**> Vec2 r σ
            <**> invDir
      tMin <- let' $ maxV2 ( min @(Code Float) <$$> t1 <**> t2 )
      tMax <- let' $ minV2 ( max @(Code Float) <$$> t1 <**> t2 )
      ...
      then pure noEvent
        ...
        dλₑ <- let' $ tMin
        xₑ  <- let' $ x ^+^ dλₑ *^ x'
        τ   <- let' =<< tilt diskInfo ( view @(Index 1) xₑ )
...
accretionDisk kerrInfo diskInfo τ constants x@(Vec4 _ r cosθ _) v = purely do
  a      <- let' $ view @(Field "a") kerrInfo
  energy <- let' $ view @(Field "energy"     ) diskInfo
  orient <- let' $ view @(Field "orientation") diskInfo
  sinθ   <- let' $ sqrt ( 1 - cosθ * cosθ )
  sinτ   <- let' $ sin τ
  cosτ   <- let' $ cos τ
  r³     <- let' $ r * r * r
  -- Very basic accretion disk temperature model: assumed to follow a -3/4 power law.
  temp   <- let' $ energy * sqrt ( invSqrt r³ ) -- r^(-3/4)
  ...
  dopp_g <- let' =<< localGravitationalDopplerFactor kerrInfo constants x

  -- Special-relativistic Doppler effect.
  ω <- let' $ signum a * orient * recip ( sqrt r³ + abs a ) -- ≈ r^(-3/2)
  let v_disk = Vec4 0 0 ( - sinτ * sinθ * ω ) ( cosτ * ω )
  dopp_s <- let' =<< specialDopplerFactor kerrInfo x v v_disk
...
tilt diskInfo r = purely do
  dip    <- let' $ 0.58
  τ      <- let' $ view @(Field "inclination") diskInfo
  r_BP   <- let' $ view @(Field "r_BP"       ) diskInfo
  s      <- let' $ r - r_BP
  s²_p_1 <- let' $ s * s + 1
  let res =
        if r < r_BP
...
rayEscaped _
  ...
  = purely do
      ...
      then do
        ...
        sinθ <- let' $ sqrt ( 1 - cosθ * cosθ )
        dθ_by_dr <- let' $ - cosθ' / ( r' * sinθ )
        dφ_by_dr <- let' $   φ'    /   r'
        sinθdφ_by_dr <- let' $ sinθ * dφ_by_dr
        cosθ_inf <- let' $ ( cosθ - r * sinθ * dθ_by_dr )
                        / sqrt ( 1 + r * r * ( dθ_by_dr * dθ_by_dr + sinθdφ_by_dr * sinθdφ_by_dr ) )
        φ_inf    <- let' $ φ + atan2 ( r * sinθdφ_by_dr ) ( sinθ + r * cosθ * dθ_by_dr )
        far <- let' =<< farField cosθ_inf φ_inf
        pure $ Event ( dλ, far )
      else
        if λ + dλ > 1e3
...
farField cosθ φ_ = purely do
  ...
  ----------------------------------
  -- stars
  off_cosθ <- let' $ ( ( cosθ `mod` 0.002 ) - 0.001 )
  off_φ    <- let' $ ( ( φ    `mod` 0.002 ) - 0.001 )
  x        <- let' $ cosθ - off_cosθ
  y        <- let' $ φ    - off_φ
  h1       <- let' $ hash (x-y) (y+x)
  h2       <- let' $ hash  x    (y-x)
