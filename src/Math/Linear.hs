...
deriving via ( V n `Compose` V m ) instance (KnownNat m, KnownNat n) => Functor     (M m n)
deriving via ( V n `Compose` V m ) instance (KnownNat m, KnownNat n) => Applicative (M m n)
