...
import qualified SPIRV.Stage as SPIRV
  ...
  , ExecutionInfo
  , Backend, OtherBackend, BackendOf
  )
...
type family Has (k :: Symbol) (i :: ProgramState) :: Type where
  Has k ('ProgramState bds _ _ _ _ _ _ _) = HasBinding k (Lookup k bds)
...
type family CanGet (k :: Symbol) (i :: ProgramState) :: Constraint where
  CanGet k ('ProgramState bds _ _ _ _ _ _ _) = GetBinding k (Lookup k bds)
...
type family CanPut (k :: Symbol) (i :: ProgramState) :: Constraint where
  CanPut k ('ProgramState bds _ _ _ _ _ _ _) = PutBinding k (Lookup k bds)
...
type family AddBinding
              ...
              ( s  :: ProgramState )
              :: ProgramState where
  AddBinding k bd ('ProgramState bds ctx cfg funs eps iface rayQueries bkend)
    = 'ProgramState (Insert k bd bds) ctx cfg funs eps iface rayQueries bkend
...
type family ValidDef (k :: Symbol) (i :: ProgramState) (a :: Type) :: Constraint where
  ValidDef k ('ProgramState bds _ _ _ _ _ _ _) a = 
...
type family FunctionDefinitionStartState
            ...
            | j -> k as
            where
  FunctionDefinitionStartState k as ('ProgramState i ctx cfg funs eps iface rayQueries bkend)
    = 'ProgramState (Union i as) ('InFunction k as) cfg funs eps iface rayQueries bkend
...
type family FunctionDefinitionEndState
            ...
            | j -> k as bds
            where
  FunctionDefinitionEndState k as bds ('ProgramState _ _ cfg funs eps iface rayQueries bkend)
    = 'ProgramState
        bds
        ( 'InFunction k as)
        cfg
...
type family GetFunctionInfo
            ...
            :: FunctionInfo
            where
  GetFunctionInfo k ('ProgramState _ _ _ funs _ _ _ _)
...
type family AddFunBinding (k :: Symbol) (as :: BindingsMap) (b :: Type) (s :: ProgramState) :: ProgramState where
  AddFunBinding k as b ('ProgramState bds ctx cfg funs eps iface rayQueries bkend)
  -- 'ValidFunDef' should have already checked that name 'k' is not already in use
    = 'ProgramState (Insert k (Fun as b) bds) ctx cfg (SetFunctionDefined k funs) eps iface rayQueries bkend
...
type family SetFunctionContext
              ...
              ( s  :: ProgramState )
              :: ProgramState where
  SetFunctionContext k as ('ProgramState bds TopLevel cfg funs eps iface rayQueries bkend)
    = 'ProgramState bds ('InFunction k as) cfg funs eps iface rayQueries bkend
  SetFunctionContext k _ ('ProgramState _ ('InFunction l _) _ _ _ _ _ _)
    = TypeError
  ...
  SetFunctionContext k _
    ( 'ProgramState _ ('InEntryPoint stageName ( _ :: SPIRV.ExecutionInfo Nat em) _) _ _ _ _ _ _ )
    = TypeError
...
type family ValidFunDef 
    ...
    :: Constraint
    where
  ValidFunDef k _ ('ProgramState _ ('InFunction l _) _ _ _ _ _ _ ) _
    = TypeError
        (    Text "'fundef': unexpected nested function definition."
        :$$: Text "Function " :<>: ShowType k
        :<>: Text " declared inside the body of function " :<>: ShowType l :<>: Text "."
  ...
  ValidFunDef k as ('ProgramState i 'TopLevel _ _ _ _ _ _) l
...
type family ValidEntryPoint
              ...
              ( l        :: BindingsMap               )
            :: Constraint where
  ValidEntryPoint k (nfo :: SPIRV.ExecutionInfo Nat s) ('ProgramState i _ _ _ eps _ _ bkend) l
...
type family EntryPointsBackend
              ( eps :: [ Symbol :-> ( SPIRV.ExecutionModel, modes ) ] )
            :: SPIRV.Backend
            where
  EntryPointsBackend '[]
    = TypeError
    (    Text "SPIR-V module must specify at least one entry point."
    :$$: Text "Entry points are specified as part of a type signature containing module definitions."
    :$$: Text "For instance, a fragment shader can be specified as follows:"
    :$$: Text " \"main\" ':-> EntryPoint '[ OriginLowerLeft ] Fragment "
    )
  EntryPointsBackend ( ( k ':-> '( s, _ ) ) ': eps )
    = CompatibleWithBackend k s (SPIRV.BackendOf s) eps

type family CompatibleWithBackend
              ( k     :: Symbol               )
            ...
            :: SPIRV.Backend
            where
  CompatibleWithBackend _ _ bk '[] = bk
  CompatibleWithBackend k s bk ( ( k' ':-> '( s', _ ) ) ': eps )
    = If ( SPIRV.BackendOf s' == bk )
        ( CompatibleWithBackend k s bk eps )
        ( TypeError
          (    Text "Incompatible backends."
          :$$: Text ( SPIRV.NamedExecutionModel k s )
          :<>: Text " uses " :<>: ShowType bk :<>: Text ","
          :$$: Text "while " :<>: Text ( SPIRV.NamedExecutionModel k' s' )
          :<>: Text " uses " :<>: ShowType ( SPIRV.OtherBackend bk ) :<>: Text "."
          )
        )
...
type family EntryPointStartState
            ...
            | j -> k nfo
            where
  EntryPointStartState k nfo ('ProgramState bds _ cfg funs eps g_iface _ bkend)
    = 'ProgramState
...
type family EntryPointEndState
            ...
            | j -> k nfo bds iface
            where
  EntryPointEndState k nfo bds iface ('ProgramState _ _ cfg funs eps g_iface rayQueries bkend)
    = 'ProgramState
        bds
...
type family GetExecutionInfo
            ...
            where
  GetExecutionInfo k em ('ProgramState _ _ _ _ eps _ _ _)
...
type family SetInterface
            ...
            :: ProgramState
            where
  SetInterface k (nfo :: SPIRV.ExecutionInfo Nat s) iface ('ProgramState i fc cfg funs eps globalIface rayQueries bkend)
    = 'ProgramState i fc cfg funs (SetInterfaceOf k s iface eps) globalIface rayQueries bkend
...
type family CanEmbed (i :: ProgramState) (j :: ProgramState) :: Constraint where
  CanEmbed ('ProgramState '[] 'TopLevel _ '[] _ _ _ _) _ = ()
  CanEmbed ('ProgramState i_bds i_ctx _ i_funs _ _ _ _) ('ProgramState j_bds i_ctx _ j_funs _ _ _ _)
    = ( SubsetBindings  i_bds  j_bds
      , SubsetFunctions i_funs j_funs
      )
  CanEmbed ('ProgramState _ i_ctx _ _ _ _ _ _) ('ProgramState _ j_ctx _ _ _ _ _ _)
    = TypeError
